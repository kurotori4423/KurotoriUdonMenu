...
public class UdonPhone : UdonSharpBehaviour
{
    ...
    private void AddComList(UdonCom com)
    {
        for (int i = 0; i < coms.Length; ++i)
        {
            if (coms[i] == null)
            {
                ...
                logSystem.AddLog(string.Format("Add UdonCom Complete. comID:{0} playerId:{1}", coms[i].comID, coms[i].GetPlayerID()));
                return;
            }
        }

        logSystem.AddLog(string.Format("Faild add UdonCom. playerID:{0}", com.GetPlayerID()));
    }
    ...
    private UdonCom GetPlayerCom(VRCPlayerApi player)
    {
        for (int i = 0; i < coms.Length; ++i)
        {
            ...
            {
                logSystem.AddLog("GetPlayerCom ComID:" + i + " player:" + player.playerId);
                return coms[i];
            }
        }
    ...
    private void RemoveCom(UdonCom com)
    {
        for (int i = 0; i < coms.Length; ++i)
        {
            ...
            {
                logSystem.AddLog(string.Format("UdonCom Destroy. comID:{0} playerID:{1}", i, com.GetPlayerID()));
    ...
    void SetConnect(VRCPlayerApi player, bool connect)
    {
        ...
        logSystem.AddLog(string.Format("SetConnect {0}:{1} : {2}", player.displayName, player.playerId, connect.ToString()));
        ...
        logSystem.AddLog(string.Format("SetConnect Complete. {0}:{1}", player.displayName, player.playerId));
    }
    ...
    void SendConnectEvent(int fromID, int toID, bool isConnect)
    {
        ...
        logSystem.AddLog("Connect: " + fromID + "->" + toID);
    ...
    public void ConnectFromTo(int from, int to)
    {
        ...
        {
            logSystem.AddLog("Connect Accept. " + from + " -> " + to);
            // 電話の受け手だったら
            ...
            if (fromPlayer != null)
            {
                ...
                logSystem.AddLog("Connect Complete. from:" + fromPlayer.playerId + ":" + from);
            }
        }
        else
        {
